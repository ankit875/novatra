...
export const handler: EventBridgeHandler<"Scheduled Event", null, void> = async (event) => {
    ...
    const market: any = await getMarket(1)
        ...
        if (currentRound > round.onchainId && ((currentRound - round.onchainId) > 1)) {
            if (round.finalizedTimestamp && !round.resolvedTimestamp) {
                ...
                const transaction = await aptos.transaction.build.simple({
                    ...
                    data: {
                        function: `0x896f7c28432dc223478a0ff3e9325d23f97e8bc261c1896eab85ee20c1f66183::generalized::resolve_market`,
                        functionArguments: [
                            MARKET_ID,
                            round.onchainId,
                            winning_outcome_ids,
...
const getMarket = async (marketId = 1) => {
    ...
    console.log("Markets fetched from DB:", markets)
